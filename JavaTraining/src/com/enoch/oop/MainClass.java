package com.enoch.oop;

public class MainClass {
	public static void main(String[] args) {
		Person jaiPerson = new Person();
		jaiPerson.name="Jai";
		jaiPerson.age=15;
		jaiPerson.address="abc";
		jaiPerson.mobile=123456;
		System.out.println(jaiPerson.name + " " + jaiPerson.age + " " + jaiPerson.address + " " + jaiPerson.mobile);{
		Person arunperson = new Person();
		arunperson.name= "arun";
		arunperson.age = 18;
		arunperson.address="xyz";
		arunperson.mobile=789123;
		System.out.println(arunperson.name + " " + arunperson.age + " " + arunperson.address + " " + arunperson.mobile);
		}
	}
}


